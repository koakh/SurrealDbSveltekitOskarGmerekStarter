<script lang="ts">
	...
	import { card, cardContent, cardIcon, cardStat, cardTitle } from '$lib/styles/card.mcss';
	...
	const totalUsers = createStore('totalUsers', data.totalUsers);
	const yourPosts = createStore('yourPosts', data.yourPosts);
	const latestUser = createStore('latestUser', data.latestUser);

	$: $latestUser;
	observeLive('user', latestUser);
	$: $posts
	observeLive('post', posts)
	$: $yourPosts
	// TODO: Bellow should observe 'yourPosts' instead of 'post', but there is the problem with permission on the 'yourPosts' view. Temporary workaround.
	observeLive('post', yourPosts)
	$: $totalUsers
	observeLive('user', totalUsers)
...
<div class="min-vh min-w:full p:20">
	<div class="flex flex:col w:98%@<2xs max-w:800 m:auto">
		<div class="grid gap:6 grid-cols:3">
			<div class={card}>
				<div class={cardIcon}><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 13a2 2 0 1 0 4 0a2 2 0 0 0-4 0m-2 8v-1a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v1M15 5a2 2 0 1 0 4 0a2 2 0 0 0-4 0m2 5h2a2 2 0 0 1 2 2v1M5 5a2 2 0 1 0 4 0a2 2 0 0 0-4 0m-2 8v-1a2 2 0 0 1 2-2h2"/></svg></div>
				<div class={cardContent}>
					<div class={cardTitle}>Total users</div>
					<div class={cardStat}>{$totalUsers.length}</div>
				</div>cardTitle
			</div>
			<div class={card}>
				<div class={cardIcon}><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 20 20"><path fill="currentColor" d="M7 0a2 2 0 0 0-2 2h9a2 2 0 0 1 2 2v12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z"/><path fill="currentColor" d="M13 20a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2zM9 5h4v5H9zM4 5h4v1H4zm0 2h4v1H4zm0 2h4v1H4zm0 2h9v1H4zm0 2h9v1H4zm0 2h9v1H4z"/></svg></div>
				<div class={cardContent}>
					<div class={cardTitle}>Total posts</div>
					<div class={cardStat}>{$posts.length}</div>
				</div>cardTitle
			</div>
			<a href={`/post/add`} title="Manage your posts">
				<div class={card}>
					<div class={cardIcon}><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M6.625 22q-1.975 0-3.363-1.388T1.875 17.25q0-1.425.738-2.55T4.55 13H2v-2h6v6H6v-2.425q-.925.2-1.525.95t-.6 1.725q0 1.15.812 1.95t1.938.8v2ZM7 9h10V7H7v2Zm3 12v-4h4v-2h-4v-2h7v-2h-7V9H3V5q0-.825.588-1.413T5 3h14q.825 0 1.413.588T21 5v14q0 .825-.588 1.413T19 21h-9Z"/></svg></div>
					<div class={cardContent}>
						<div class={cardTitle}>Your posts</div>
						<div class={cardStat}>{$yourPosts.length}</div>
					</div>cardTitle
				</div>
			</a>
				<div class="grid-col-span:3 bg:yellow-70 flex fg:gray-20 p:10">
					...
					<div class="px:6 overflow:hidden white-space:nowrap text-overflow:ellipsis as:center flex"><a href={link_user_prefix+$latestUser[0].slug} title={$latestUser[0].username} class="fg:black:hover">{$latestUser[0].username}</a><p class="px:3 hidden@<2xs"> joined {dayjs($latestUser[0].created_at).fromNow()}</p></div>
				</div>
		</div>
